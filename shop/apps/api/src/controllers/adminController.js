...
exports.getPendingKYC = async (req, res, next) => {
  try {
    ...
    const combined = [
      ...
      ...affiliates.map(a => ({ ...a, type: 'affiliate' })),
    ...
    res.json({
      ...
      data: combined,
      meta: {
        total: totalVendors + totalAffiliates,
        totalVendors,
        totalAffiliates,
        page: parseInt(page),
        limit: parseInt(limit),
...
exports.approveVendorKYC = async (req, res, next) => {
  try {
    ...
    logger.info(`Vendor KYC approved: ${vendor.storeName} by admin`);
...
exports.rejectVendorKYC = async (req, res, next) => {
  try {
    ...
    vendor.kyc.verifiedAt = undefined;
    await vendor.save();

    logger.info(`Vendor KYC rejected: ${vendor.storeName} - Reason: ${reason}`);
...
exports.approveAffiliateKYC = async (req, res, next) => {
  try {
    ...
    affiliate.kyc.verifiedAt = new Date();
    affiliate.kyc.rejectionReason = undefined;
    affiliate.status = 'active';
    await affiliate.save();

    logger.info(`Affiliate KYC approved: ${affiliate.code} by admin`);

    res.json({
      success: true,
      data: affiliate,
      message: 'Affiliate KYC approved successfully',
    });
  } catch (error) {
    next(error);
  }
...
exports.rejectAffiliateKYC = async (req, res, next) => {
  try {
    ...
    affiliate.kyc.rejectionReason = reason;
    affiliate.kyc.verifiedAt = undefined;
    await affiliate.save();

    logger.info(`Affiliate KYC rejected: ${affiliate.code} - Reason: ${reason}`);

    res.json({
      success: true,
      data: affiliate,
      message: 'Affiliate KYC rejected',
    });
  } catch (error) {
    next(error);
  }
