...
const CartDrawer = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform flex flex-col animate-slide-in-right">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div className="flex items-center gap-2">
            <ShoppingCart className="w-5 h-5 text-blue-600" />
            <h2 className="text-xl font-bold text-gray-900">Shopping Cart</h2>
            <span className="bg-primary-600 text-white text-xs font-bold px-2 py-1 rounded-full cart-count-update">
              {items.length}
            </span>
          </div>
          <button
