...
export default function VendorKYC() {
  const { isReady, user } = useAuthGuard([ROLES.VENDOR, ROLES.ADMIN]);
  ...
  const handleSubmitKYC = async () => {
    ...
    if (!gstNumber.trim()) {
      Alert.alert('Required', 'Please enter your GST number');
      return;
    }
    ...
    >
      <View style={styles.statusCard}>
        ...
        <Text style={styles.statusDesc}>
          {kycStatus === 'pending' && 'Submit your business documents to start selling on V-Tech.'}
          {kycStatus === 'submitted' && 'Your documents are being reviewed. This usually takes 1-2 business days.'}
          {kycStatus === 'approved' && 'Your identity has been verified. You can now sell on V-Tech.'}
          {kycStatus === 'rejected' && 'Your documents were rejected. Please re-submit with valid details.'}
        </Text>
      </View>
